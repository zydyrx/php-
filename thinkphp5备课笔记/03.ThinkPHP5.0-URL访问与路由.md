...
例如，我们希望使用user可以访问index模块的User控制器的所有操作，可以使用：Route::alias('user','index/User');在浏览器中访问index/User下所有的方法时，可以使用:入口文件/user/方法名就行.
注意:此处别参考手册内容,完全说错.如:
```php
// user 别名路由到 index/User 模块/控制器
Route::alias('user','index/User');//在5.0.20中这里不是: 模块/控制器  而是 : 控制器/方法 但在5.0.24中测试完全OK。
```

ThinkPHP5.0.20还非常不稳定,路由别名有BUG. 在5.0.24中就OK.
```php
...
### 资源路由
语法:Route::resource('路由','模块/控制器'); 不需要写方法,一条语句自动生成7条路由,但save方法必须用post方式访问,update必须用 put方式访问,delete必须用delete方式访问,不然它们访问到的都是read()这个方法.详细见下面表格.
```php
...
//当你访问http://serverName/blog/create   URL访问的是create方法
实例:
```php
//=========资源路由配置===============
