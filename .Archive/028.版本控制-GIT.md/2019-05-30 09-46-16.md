...
1. 我们下载的是3.1.3版本,安装需要.net framework 4.7.1支持,所以我们要先下载安装了  .net framework 4.7.2
2. sourcetree双击即安装完成,但进不去.需要配置才能进去.
3. 提供两个配置文件:accounts.json user.config  就能进去了  
    3.1 accounts.json文件内容如下:保存到:
```shell
[
  {
    "$id": "14",
