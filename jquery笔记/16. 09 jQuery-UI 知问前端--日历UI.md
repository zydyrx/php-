...
altField : '#abc',//指定三方input填充日期
altFormat : 'yy-mm-dd',//给指定三方input指定格式
appendText : '(yy-mm-dd)',//在指定input后面个span标签内有文本
firstDay : 1,//表示日成从星期几开始,0表示周日
showWeek : true,//是否第一坚排显示周
weekHeader : '周',//日历第一坚排的周标题
});
```
datepicker 外观选项
```table
