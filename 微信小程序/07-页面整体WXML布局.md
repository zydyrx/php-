...
```
在wx:for的语句中,一般要加上 wx:key={{index}},否则浏览器有个警告,注意wx:key的值必须是唯一的.
```xml
<!--wxml-->
...
## [条件渲染](https://mp.weixin.qq.com/debug/wxadoc/dev/framework/view/wxml/conditional.html)  wx:if  wx:elif   wx:else 
