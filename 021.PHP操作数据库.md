...
//注意:如果要写入NULL值,请把变量不赋值或赋值为NULL,它插入到数据库时就是NULL值.
...
### 使用示例
```php
$dns = 'mysql:host=localhost;dbname=kp10b_blog';
$db_user = 'root';
$db_pwd = 'root';
	//1.创建PDO对象,相当于打开数据库连接.
...
```php
//方法与过程一样,记得查询获取结果就行
//$stmt->setFetchMode(PDO::FETCH_ASSOC);//设置结果集数组的格式.
